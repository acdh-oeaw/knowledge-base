...
interface DashboardReportingCountryReportSummaryPageProps {
	params: Promise<{ year: string; slug: string }>;
}

export async function generateMetadata(
	_props: Readonly<DashboardReportingCountryReportSummaryPageProps>,
	resolvingMetadata: ResolvingMetadata,
): Promise<Metadata> {
	const t = await getExtracted();

	return createMetadata(resolvingMetadata, {
		title: t("Dashboard - Country report summary"),
	});
}

export default async function DashboardReportingCountryReportSummaryPage(
	props: Readonly<DashboardReportingCountryReportSummaryPageProps>,
): Promise<ReactNode> {
	const { params } = props;
