...
/**
 * Resolve, per page document, the published version to prefer: the requested/default locale's
	...
	const [items, aggregate] = await Promise.all([
		db
			.select({
				...
				imageCaption: schema.pages.imageCaption,
				imageCaptionMode: schema.pages.imageCaptionMode,
			...
			.orderBy(desc(schema.pages.publicationDate), desc(schema.entityVersions.updatedAt))
			.limit(limit)
			.offset(offset),
		db
			.select({ total: count() })
