...
async function main() {
	...
	async function ensureInstitutionOrgUnit(
		...
		const slugExists = await tx.query.slugs.findFirst({
			where: {
	...
	for (const page of Object.values(data.pages)) {
		...
		await db.transaction(async (tx) => {
			...
			const pageSlug = normalizeWordPressSlug(page.slug, toPlaintext(page.title.rendered));
	...
	for (const { group, items } of documents) {
		...
		for (const item of items) {
			await db.transaction(async (tx) => {
				const documentPolicySlug = slugify(item.title);
	...
	for (const page of Object.values(data.initiatives)) {
		...
		await db.transaction(async (tx) => {
			const initiativeSlug = normalizeWordPressSlug(page.slug, toPlaintext(page.title.rendered));
	...
	for (const post of Object.values(data.posts)) {
		...
		await db.transaction(async (tx) => {
			const newsSlug = normalizeWordPressSlug(post.slug, toPlaintext(post.title.rendered));
	...
	for (const event of Object.values(data.events)) {
		...
		await db.transaction(async (tx) => {
			const eventSlug = normalizeWordPressSlug(event.slug, toPlaintext(event.title));
	...
	for (const project of Object.values(data.projects)) {
		...
		await db.transaction(async (tx) => {
			...
			if (projectId == null) {
				const projectSlug = normalizeWordPressSlug(
					project.slug,
					toPlaintext(project.title.rendered),
				);
