CREATE TABLE "audit_logs" (
	...
	CONSTRAINT "audit_logs_action_enum_check" CHECK ("action" in ('create', 'update', 'delete', 'publish', 'discard_draft', 'launch', 'close', 'sync', 'ingest', 'relation_create', 'relation_end', 'impersonation_start', 'impersonation_end'))
);
...
--> statement-breakpoint
CREATE TABLE "content_blocks_type_accordion_item" (
	"id" uuid PRIMARY KEY,
	"title" text NOT NULL,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL
...
--> statement-breakpoint
CREATE TABLE "content_blocks_type_callout" (
	"id" uuid PRIMARY KEY,
	"intent" text DEFAULT 'info' NOT NULL,
	"title" text,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "content_blocks_type_callout_intent_enum_check" CHECK ("intent" in ('neutral', 'info', 'warning', 'danger', 'success'))
);
--> statement-breakpoint
CREATE TABLE "content_blocks_types" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "content_blocks_types_type_enum_check" CHECK ("type" in ('accordion', 'accordion_item', 'callout', 'data', 'embed', 'gallery', 'hero', 'image', 'media_text', 'rich_text'))
);
--> statement-breakpoint
...
CREATE TABLE "content_blocks_type_gallery_items" (
	...
	"position" integer NOT NULL,
	"caption" jsonb,
	"caption_mode" text DEFAULT 'inherit' NOT NULL,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "content_blocks_type_gallery_items_caption_mode_enum_check" CHECK ("caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
...
CREATE TABLE "content_blocks_type_hero" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "content_blocks_type_hero_caption_mode_enum_check" CHECK ("caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
CREATE TABLE "content_blocks_type_image" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "content_blocks_type_image_caption_mode_enum_check" CHECK ("caption_mode" in ('hidden', 'inherit', 'override')),
	CONSTRAINT "content_blocks_type_image_layout_enum_check" CHECK ("layout" in ('default', 'wide', 'full', 'float-start', 'float-end'))
);
--> statement-breakpoint
CREATE TABLE "content_blocks_type_media_text" (
	"id" uuid PRIMARY KEY,
	"image_id" uuid NOT NULL,
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "content_blocks_type_media_text_side_enum_check" CHECK ("side" in ('start', 'end')),
	CONSTRAINT "content_blocks_type_media_text_caption_mode_enum_check" CHECK ("caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
...
CREATE TABLE "entity_types" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "entity_types_type_enum_check" CHECK ("type" in ('documentation_pages', 'documents_policies', 'events', 'funding_calls', 'impact_case_studies', 'internal_pages', 'news', 'opportunities', 'organisational_units', 'pages', 'persons', 'projects', 'spotlight_articles'))
);
--> statement-breakpoint
...
CREATE TABLE "entity_versions" (
	...
	CONSTRAINT "entity_versions_entity_id_status_id_locale_id_unique" UNIQUE("entity_id","status_id","locale_id")
);
--> statement-breakpoint
CREATE TABLE "fields" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
...
CREATE TABLE "events" (
	...
	"website" text,
	"image_caption" jsonb,
	"image_caption_mode" text DEFAULT 'inherit' NOT NULL,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "events_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
CREATE TABLE "funding_calls" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "funding_calls_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
CREATE TABLE "impact_case_studies" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "impact_case_studies_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
...
CREATE TABLE "news" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "news_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
CREATE TABLE "opportunities" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "opportunities_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
...
CREATE TABLE "pages" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "pages_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
CREATE TABLE "person_role_types" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "person_role_types_type_enum_check" CHECK ("type" in ('is_affiliated_with', 'is_chair_of', 'is_vice_chair_of', 'is_member_of', 'is_contact_for', 'is_content_manager_for', 'national_coordinator', 'national_coordinator_deputy', 'national_coordination_staff', 'national_representative', 'national_representative_deputy'))
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE "person_social_media_types" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
	"type" text NOT NULL UNIQUE,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "person_social_media_types_type_enum_check" CHECK ("type" in ('academia_edu', 'bluesky', 'github', 'gitlab', 'google_scholar', 'humanities_commons', 'hypotheses', 'linkedin', 'mastodon', 'researchgate', 'twitter', 'website', 'youtube', 'zenodo', 'other'))
);
--> statement-breakpoint
CREATE TABLE "persons" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "persons_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
...
CREATE TABLE "country_report_contributions" (
	...
	CONSTRAINT "country_report_contributions_contribution_role_enum_check" CHECK ("contribution_role" in ('national_coordinator', 'national_coordinator_deputy', 'is_chair_of_jrc', 'is_chair_of_ncc', 'is_chair_of_wg', 'is_member_of_jrc'))
);
--> statement-breakpoint
CREATE TABLE "country_report_institutions" (
	...
	CONSTRAINT "country_report_institutions_report_unit_document_type_unique" UNIQUE("country_report_id","organisational_unit_document_id","representation_type"),
...
--> statement-breakpoint
CREATE TABLE "country_report_services" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
	"country_report_id" uuid NOT NULL,
	"service_id" uuid NOT NULL,
	CONSTRAINT "country_report_services_country_report_id_service_id_unique" UNIQUE("country_report_id","service_id")
);
--> statement-breakpoint
CREATE TABLE "country_report_social_media" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
	"country_report_id" uuid NOT NULL,
	"social_media_id" uuid NOT NULL,
	CONSTRAINT "country_report_social_media_country_report_id_social_media_id_unique" UNIQUE("country_report_id","social_media_id")
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE "report_external_resource_snapshot_items" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
	...
	"year" integer,
	"pid" text,
	CONSTRAINT "report_external_resource_snapshot_items_snapshot_document_unique" UNIQUE("snapshot_id","search_document_id"),
	CONSTRAINT "report_external_resource_snapshot_items_snapshot_position_unique" UNIQUE("snapshot_id","position")
);
--> statement-breakpoint
CREATE TABLE "report_external_resource_snapshots" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "report_external_resource_snapshots_country_report_section_unique" UNIQUE("country_report_id","section"),
	CONSTRAINT "report_external_resource_snapshots_working_group_report_section_unique" UNIQUE("working_group_report_id","section"),
	CONSTRAINT "report_external_resource_snapshots_section_enum_check" CHECK ("section" in ('country_sshoc_resources', 'country_zotero_publications', 'working_group_sshoc_resources', 'working_group_zotero_publications')),
	CONSTRAINT "report_external_resource_snapshots_report_owner_xor_check" CHECK (
				(
...
--> statement-breakpoint
CREATE TABLE "working_group_report_chairs" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
	"working_group_report_id" uuid NOT NULL,
	"person_to_org_unit_id" uuid NOT NULL,
	"chair_role" text NOT NULL,
	CONSTRAINT "working_group_report_chairs_working_group_report_id_person_to_org_unit_id_unique" UNIQUE("working_group_report_id","person_to_org_unit_id"),
	CONSTRAINT "working_group_report_chairs_role_enum_check" CHECK ("chair_role" in ('is_chair_of', 'is_vice_chair_of'))
);
--> statement-breakpoint
...
CREATE TABLE "services" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
	"name" text NOT NULL,
	"sshoc_marketplace_id" text UNIQUE,
	"type_id" uuid NOT NULL,
	"status_id" uuid NOT NULL,
	"comment" text,
...
CREATE TABLE "social_media_types" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "social_media_types_type_enum_check" CHECK ("type" in ('bluesky', 'facebook', 'flickr', 'instagram', 'linkedin', 'mastodon', 'twitter', 'vimeo', 'website', 'youtube', 'other'))
);
--> statement-breakpoint
CREATE TABLE "spotlight_articles" (
	...
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "spotlight_articles_image_caption_mode_enum_check" CHECK ("image_caption_mode" in ('hidden', 'inherit', 'override'))
);
--> statement-breakpoint
...
CREATE TABLE "sessions" (
	...
	"impersonation_expires_at" timestamp(3) with time zone,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "sessions_impersonation_expiry_check" CHECK (
					"impersonated_user_id" IS NULL
					OR "impersonation_expires_at" IS NOT NULL
				),
	CONSTRAINT "sessions_impersonation_not_self_check" CHECK ("impersonated_user_id" IS DISTINCT FROM "user_id")
);
--> statement-breakpoint
CREATE TABLE "users" (
	"id" uuid PRIMARY KEY DEFAULT UUIDV7(),
...
CREATE INDEX "content_blocks_parent_block_id_idx" ON "content_blocks" ("parent_block_id");--> statement-breakpoint
...
CREATE INDEX "sessions_impersonated_user_id_idx" ON "sessions" ("impersonated_user_id");--> statement-breakpoint
...
ALTER TABLE "audit_logs" ADD CONSTRAINT "audit_logs_impersonated_by_user_id_users_id_fkey" FOREIGN KEY ("impersonated_by_user_id") REFERENCES "users"("id") ON DELETE SET NULL;--> statement-breakpoint
...
ALTER TABLE "content_blocks_type_accordion_item" ADD CONSTRAINT "content_blocks_type_accordion_item_id_content_blocks_id_fkey" FOREIGN KEY ("id") REFERENCES "content_blocks"("id") ON DELETE CASCADE;--> statement-breakpoint
ALTER TABLE "content_blocks_type_callout" ADD CONSTRAINT "content_blocks_type_callout_id_content_blocks_id_fkey" FOREIGN KEY ("id") REFERENCES "content_blocks"("id") ON DELETE CASCADE;--> statement-breakpoint
...
ALTER TABLE "content_blocks" ADD CONSTRAINT "content_blocks_parent_block_id_content_blocks_id_fkey" FOREIGN KEY ("parent_block_id") REFERENCES "content_blocks"("id") ON DELETE CASCADE;--> statement-breakpoint
...
ALTER TABLE "content_blocks_type_media_text" ADD CONSTRAINT "content_blocks_type_media_text_id_content_blocks_id_fkey" FOREIGN KEY ("id") REFERENCES "content_blocks"("id") ON DELETE CASCADE;--> statement-breakpoint
ALTER TABLE "content_blocks_type_media_text" ADD CONSTRAINT "content_blocks_type_media_text_image_id_assets_id_fkey" FOREIGN KEY ("image_id") REFERENCES "assets"("id");--> statement-breakpoint
...
ALTER TABLE "funding_calls" ADD CONSTRAINT "funding_calls_image_id_assets_id_fkey" FOREIGN KEY ("image_id") REFERENCES "assets"("id");--> statement-breakpoint
...
ALTER TABLE "navigation_items" ADD CONSTRAINT "navigation_items_locale_id_locales_id_fkey" FOREIGN KEY ("locale_id") REFERENCES "locales"("id");--> statement-breakpoint
...
ALTER TABLE "opportunities" ADD CONSTRAINT "opportunities_image_id_assets_id_fkey" FOREIGN KEY ("image_id") REFERENCES "assets"("id");--> statement-breakpoint
...
ALTER TABLE "person_social_media" ADD CONSTRAINT "person_social_media_person_id_persons_id_fkey" FOREIGN KEY ("person_id") REFERENCES "persons"("id");--> statement-breakpoint
ALTER TABLE "person_social_media" ADD CONSTRAINT "person_social_media_type_id_person_social_media_types_id_fkey" FOREIGN KEY ("type_id") REFERENCES "person_social_media_types"("id");--> statement-breakpoint
...
ALTER TABLE "country_report_services" ADD CONSTRAINT "country_report_services_ctbNtUQQgoOr_fkey" FOREIGN KEY ("country_report_id") REFERENCES "country_reports"("id");--> statement-breakpoint
ALTER TABLE "country_report_services" ADD CONSTRAINT "country_report_services_service_id_services_id_fkey" FOREIGN KEY ("service_id") REFERENCES "services"("id");--> statement-breakpoint
ALTER TABLE "country_report_social_media" ADD CONSTRAINT "country_report_social_media_AbJxHpzERcqB_fkey" FOREIGN KEY ("country_report_id") REFERENCES "country_reports"("id");--> statement-breakpoint
ALTER TABLE "country_report_social_media" ADD CONSTRAINT "country_report_social_media_sYwks9FzZNOc_fkey" FOREIGN KEY ("social_media_id") REFERENCES "social_media"("id");--> statement-breakpoint
...
ALTER TABLE "report_external_resource_snapshot_items" ADD CONSTRAINT "report_external_resource_snapshot_items_Wo45TLKVvzZe_fkey" FOREIGN KEY ("snapshot_id") REFERENCES "report_external_resource_snapshots"("id") ON DELETE CASCADE;--> statement-breakpoint
ALTER TABLE "report_external_resource_snapshots" ADD CONSTRAINT "report_external_resource_snapshots_jZER9seKelp6_fkey" FOREIGN KEY ("country_report_id") REFERENCES "country_reports"("id");--> statement-breakpoint
ALTER TABLE "report_external_resource_snapshots" ADD CONSTRAINT "report_external_resource_snapshots_o1STkE6vQArM_fkey" FOREIGN KEY ("working_group_report_id") REFERENCES "working_group_reports"("id");--> statement-breakpoint
ALTER TABLE "report_external_resource_snapshots" ADD CONSTRAINT "report_external_resource_snapshots_NasGyTukAi7H_fkey" FOREIGN KEY ("captured_by_user_id") REFERENCES "users"("id");--> statement-breakpoint
...
ALTER TABLE "working_group_report_chairs" ADD CONSTRAINT "working_group_report_chairs_ivj1oi6O3xWM_fkey" FOREIGN KEY ("working_group_report_id") REFERENCES "working_group_reports"("id");--> statement-breakpoint
ALTER TABLE "working_group_report_chairs" ADD CONSTRAINT "working_group_report_chairs_4ZDo2hxe6pyY_fkey" FOREIGN KEY ("person_to_org_unit_id") REFERENCES "persons_to_organisational_units"("id");--> statement-breakpoint
...
ALTER TABLE "slugs" ADD CONSTRAINT "slugs_locale_id_locales_id_fkey" FOREIGN KEY ("locale_id") REFERENCES "locales"("id");--> statement-breakpoint
...
ALTER TABLE "tenancies" ADD CONSTRAINT "tenancies_tenant_id_entities_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "entities"("id");--> statement-breakpoint
...
ALTER TABLE "sessions" ADD CONSTRAINT "sessions_impersonated_user_id_users_id_fkey" FOREIGN KEY ("impersonated_user_id") REFERENCES "users"("id") ON DELETE SET NULL;--> statement-breakpoint
